import React from 'react';
import { Head } from '@inertiajs/react';
import AppLayout from '@/Layouts/AppLayout';
import { MapPin, Briefcase, Globe, Mail, Calendar, Building } from 'lucide-react';

export default function Show({ professional }) {
    return (
        <AppLayout>
            <Head title={`${professional.name} - Professional Profile`} />

            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {/* Left Column - Profile Info */}
                    <div className="lg:col-span-1 space-y-6">
                        {/* Profile Card */}
                        <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700/50">
                            <div className="text-center mb-6">
                                <div className="relative inline-block">
                                    <img
                                        src={professional.avatar_url || "/images/default-avatar.png"}
                                        alt={professional.name}
                                        className="w-32 h-32 rounded-full object-cover ring-4 ring-white dark:ring-slate-700 mx-auto"
                                    />
                                </div>
                                <h1 className="text-2xl font-bold text-slate-900 dark:text-white mt-4">
                                    {professional.name}
                                </h1>
                                {professional.specialization && (
                                    <p className="text-slate-600 dark:text-slate-400">
                                        {professional.specialization}
                                    </p>
                                )}
                            </div>

                            <div className="space-y-4">
                                {professional.location && (
                                    <div className="flex items-center gap-3 text-slate-600 dark:text-slate-400">
                                        <MapPin className="w-5 h-5" />
                                        <span>{professional.location}</span>
                                    </div>
                                )}
                                {professional.job_category && (
                                    <div className="flex items-center gap-3 text-slate-600 dark:text-slate-400">
                                        <Briefcase className="w-5 h-5" />
                                        <span>{professional.job_category.name}</span>
                                    </div>
                                )}
                                {professional.website_url && (
                                    <div className="flex items-center gap-3 text-slate-600 dark:text-slate-400">
                                        <Globe className="w-5 h-5" />
                                        <a
                                            href={professional.website_url}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="hover:text-primary"
                                        >
                                            Portfolio Website
                                        </a>
                                    </div>
                                )}
                                <div className="flex items-center gap-3 text-slate-600 dark:text-slate-400">
                                    <Mail className="w-5 h-5" />
                                    <a
                                        href={`mailto:${professional.email}`}
                                        className="hover:text-primary"
                                    >
                                        {professional.email}
                                    </a>
                                </div>
                            </div>
                        </div>

                        {/* Skills */}
                        {professional.skills.length > 0 && (
                            <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700/50">
                                <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">
                                    Skills
                                </h2>
                                <div className="flex flex-wrap gap-2">
                                    {professional.skills.map((skill, index) => (
                                        <span
                                            key={index}
                                            className="px-3 py-1 rounded-full text-sm font-medium
                                                bg-primary-50 dark:bg-primary-900/20 
                                                text-primary-600 dark:text-primary-400"
                                        >
                                            {skill}
                                        </span>
                                    ))}
                                </div>
                            </div>
                        )}
                    </div>

                    {/* Right Column - Main Content */}
                    <div className="lg:col-span-2 space-y-8">
                        {/* About */}
                        {professional.bio && (
                            <section className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700/50">
                                <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-4">
                                    About
                                </h2>
                                <p className="text-slate-600 dark:text-slate-400 whitespace-pre-line">
                                    {professional.bio}
                                </p>
                            </section>
                        )}

                        {/* Work Experience */}
                        {professional.work_experiences.length > 0 && (
                            <section className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700/50">
                                <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-6">
                                    Work Experience
                                </h2>
                                <div className="space-y-6">
                                    {professional.work_experiences.map((exp, index) => (
                                        <div key={index} className="flex gap-4">
                                            <div className="flex-shrink-0">
                                                <Building className="w-6 h-6 text-slate-400" />
                                            </div>
                                            <div>
                                                <h3 className="text-lg font-semibold text-slate-900 dark:text-white">
                                                    {exp.title}
                                                </h3>
                                                <p className="text-slate-600 dark:text-slate-400">
                                                    {exp.company}
                                                </p>
                                                <p className="text-sm text-slate-500 dark:text-slate-500">
                                                    {exp.start_date} - {exp.end_date || "Present"}
                                                </p>
                                                {exp.description && (
                                                    <p className="mt-2 text-slate-600 dark:text-slate-400">
                                                        {exp.description}
                                                    </p>
                                                )}
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            </section>
                        )}
                    </div>
                </div>
            </div>
        </AppLayout>
    );
} 